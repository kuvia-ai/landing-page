  ...
  const handlePredictionClick = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (goToPrediction) {
          goToPrediction.scrollIntoView({ behavior: "smooth", block: "center" });
        }
      }, 100)
    }, 4100);
  };
      ...
      {selectedPatient && (
        <div className="processing-page-content">
          <div className="processing-page-images-layout">
            <div
              className={`patient-tissue-unprocessed ${isPredictionComplete ? "end" : ""
                }`}
            >
          ...
          {!isPredictionComplete && !isLoading ? (
            ...
            />
          ) : isLoading ? <ProcessingLoader style={{ marginTop: '10px' }} /> : (
            <div id="patient-prediction" className="patient-prediction">{`PREDICTION: ${patient_data.filter(
              (data) => data.patient_name === selectedPatient
            )[0].prediction
              }`}</div>
          )}
